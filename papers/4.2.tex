...
例子 4.1. \emph{我们选取节点标签和邻接节点各个标号的个数作为度量特征。所以我们一共有四个特征。这个图的特征仅属于取值中的一种:nominal。我们首先写出节点特征矩阵如图表1中的(B)。然后用小波函数提取出实际特征值。举例而言，对于$P_{3}$在用$h=0$的小波分析提取后，局部特征是$\lbrack b,2,0,1\rbrace $。然后生成的哈希表如图表1(C)所示。}  \\ \\
\end{minipage} 
\subfigure[P]{   
